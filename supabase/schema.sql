...
begin
  ...
  loop
    ...
    execute format('create policy "Users can view own records" on public.%I for select using (
      public.is_approved_user()
      and (
        auth.uid() = user_id
        or (
          case
            when t = any (array['expenses', 'owner_reports', 'tax_reserve_records'])
              then public.has_active_property_financial_access(user_id, property_id, null)
            else public.has_active_property_access(user_id, property_id, case when t = 'properties' then id else null end)
          end
        )
      )
    )', t);
    ...
    execute format('create policy "Users can insert own records" on public.%I for insert with check (
      public.is_approved_user()
      and (
        auth.uid() = user_id
        or public.has_active_property_operations_edit_access(user_id, property_id, case when t = 'properties' then id else null end)
      )
    )', t);

    execute format('drop policy if exists "Users can update own records" on public.%I', t);
    execute format('create policy "Users can update own records" on public.%I for update using (
      public.is_approved_user()
      and (
        auth.uid() = user_id
        or public.has_active_property_operations_edit_access(user_id, property_id, case when t = 'properties' then id else null end)
    ...
    execute format('create policy "Users can delete own records" on public.%I for delete using (
      public.is_approved_user()
      and (
        auth.uid() = user_id
        or public.has_active_property_operations_edit_access(user_id, property_id, case when t = 'properties' then id else null end)
