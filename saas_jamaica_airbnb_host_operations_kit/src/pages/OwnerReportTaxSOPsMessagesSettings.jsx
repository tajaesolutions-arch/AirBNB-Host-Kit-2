...
export function Settings() {
  const { properties, setProperties, settings, setSettings, resetToSampleData, clearAllData, saving } = useApp();
  ...
  const updateAccount = async (k, v) => {
    try { await updateProfile({ ...(profile || {}), [k]: v }); } catch (err) { console.error(err); }
  };
  ...
  return (
    <div className="page">
      ...
      <div className="card" style={{ padding: 22, marginBottom: 22, border: "1px solid #9FD8CF" }}>
        <SectionTitle>Account & Cloud Storage</SectionTitle>
        <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 14 }}>You are logged in as <strong>{user?.email}</strong>. Your dashboard data is saved privately to your Supabase cloud database. Other hosts cannot see your records.</p>
        <div className="form-grid-2">
          <div className="field"><label className="field-label">Account Host Name</label><input value={profile?.host_name || ""} onChange={e => updateAccount("host_name", e.target.value)} /></div>
          <div className="field"><label className="field-label">Account Business Name</label><input value={profile?.business_name || ""} onChange={e => updateAccount("business_name", e.target.value)} /></div>
          <div className="field"><label className="field-label">Account Phone</label><input value={profile?.phone || ""} onChange={e => updateAccount("phone", e.target.value)} /></div>
          <div className="field"><label className="field-label">Login Email</label><input value={user?.email || ""} disabled /></div>
        </div>
      </div>
      ...
      {/* Data tools */}
      <div className="card" style={{ padding: 22, background: "var(--sand-soft)", border: "1px solid var(--line-strong)" }}>
        <div style={{ fontWeight: 700, marginBottom: 6, color: "var(--navy)" }}>Data Tools</div>
        <p style={{ fontSize: 13, color: "var(--muted)", marginBottom: 14 }}>Use sample data to test the dashboard, or clear your account when you want a blank workspace. These actions only affect the logged-in account.</p>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button className="btn-primary" disabled={saving} onClick={() => { if (window.confirm("Load sample data into this account? This will replace current dashboard records.")) resetToSampleData(); }}>Load Sample Data</button>
          <button className="btn-danger" disabled={saving} onClick={() => { if (window.confirm("Clear all properties, bookings, guests, tasks, supplies, expenses, and leads from this account?")) clearAllData(); }}>Clear Account Data</button>
        </div>
      </div>
