...
const SETUP_CHECKLIST_COLLAPSED_STORAGE_KEY = "jak_setup_checklist_collapsed";
...
function SetupProgressCard({ checklist, onGoToPage, onMarkComplete, onMarkSkipped }) { const [isOpen, setIsOpen] = useState(() => localStorage.getItem(SETUP_CHECKLIST_OPEN_STORAGE_KEY) !== "false"); const [isCollapsed, setIsCollapsed] = useState(() => localStorage.getItem(SETUP_CHECKLIST_COLLAPSED_STORAGE_KEY) === "true"); const completedCount = checklist.filter((item) => item.completed).length; const percent = checklist.length ? Math.round((completedCount / checklist.length) * 100) : 0; if (percent === 100) return null; const toggle = () => setIsOpen((p) => { const n = !p; localStorage.setItem(SETUP_CHECKLIST_OPEN_STORAGE_KEY, String(n)); return n; }); const toggleCollapsed = () => setIsCollapsed((prev) => { const next = !prev; localStorage.setItem(SETUP_CHECKLIST_COLLAPSED_STORAGE_KEY, String(next)); return next; });
  return <div className="card dashboard-overview dashboard-setup-card"><div className="dashboard-card-header dashboard-setup-card-header"><h3>Setup Checklist</h3><div className="dashboard-setup-meta"><span>{completedCount}/{checklist.length} complete</span><button type="button" className="btn-ghost setup-mini-btn" onClick={toggleCollapsed}>{isCollapsed ? "Show" : "Hide"}</button><button type="button" className="btn-ghost setup-mini-btn" onClick={toggle} aria-label="Toggle setup checklist">{isOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</button></div></div><div className="setup-bar"><div className="setup-fill" style={{ width: `${percent}%` }} /></div>{!isCollapsed && isOpen && <div className="dashboard-setup-items">{checklist.map((item) => <div key={item.id} className="dashboard-list-item dashboard-setup-item-row"><span className="dashboard-setup-title">{item.completed ? <CheckCircle2 size={14} /> : <Circle size={14} />}{item.title}</span><div className="dashboard-setup-actions">{!item.completed && item.canManuallyComplete && <button type="button" className="btn-ghost setup-mini-btn" onClick={() => onMarkComplete(item.id)}>Done</button>}{!item.completed && item.canManuallyComplete && <button type="button" className="btn-ghost setup-mini-btn" onClick={() => onMarkSkipped(item.id)}>Skip</button>}<button type="button" className="btn-ghost setup-mini-btn" onClick={() => onGoToPage(item.page)}>{item.action}</button></div></div>)}</div>}</div>; }
